...
def parse_filename(path: Path) -> tuple[str, int | str, int, int] | None:
    """Return (mode, a, b, run). Mode is 'pg', 'cxmu', 't', 'train', or 'nodes'."""
...
def run(exp_dir: Path, out_dir: Path, dpi: int) -> None:
    ...
    if not groups:
        raise SystemExit(
            ...
            "Use exp_p*_g*_*.csv, exp_cx*_mu*_*.csv, exp_t*_*.csv, exp_train*_*.csv, or exp_*_<letter>_<run>.csv."
        )
    ...
    # 4) Ranking: weighted and unweighted mean % vs NN (across runs)
    cfg_to_uw = {c: (mw, sw) for c, mw, sw in zip(col_keys, means_nn_uw, stds_nn_uw)}
    rank_nn = sorted(
        zip(col_keys, means_nn, stds_nn),
        key=lambda t: t[1],
        reverse=True,
    )
    print("\nRanking by mean % vs NN (higher is better; ordered by weighted mean):")
    for i, (cfg, m_w, s_w) in enumerate(rank_nn, 1):
        m_uw, s_uw = cfg_to_uw[cfg]
        lab = cfg_label(cfg).replace("\n", " ")
        print(
            f"  {i}. {lab}:  weighted {m_w:+.3f}% ± {s_w:.3f}  |  "
            f"unweighted {m_uw:+.3f}% ± {s_uw:.3f}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory containing exp_p*_g*_*.csv, exp_cx*_mu*_*.csv, exp_t*_*.csv, exp_train*_*.csv, or node_size exp_*_<letter>_* files",
    )
    parser.add_argument(
        "--out_dir",
        type=Path,
